...
const CanvasEditor = {
  ...
  // ── Drag de parede com paredes adjacentes ─

  // Constrói o estado completo do drag incluindo quais paredes adjacentes devem seguir.
  // Tolerância de snap: 30mm — cobre pequenas imprecisões de conexão entre paredes.
  _buildWallDrag(w, mode, startWorld) {
    const SNAP  = 30;
    const walls = this.project.canvas.walls;

    const collectAt = (ox, oy) => {
      const list = [];
      for (const other of walls) {
        if (other.id === w.id) continue;
        if (dist(other.x1, other.y1, ox, oy) <= SNAP) list.push({ id: other.id, ep: 'p1', ox: other.x1, oy: other.y1 });
        if (dist(other.x2, other.y2, ox, oy) <= SNAP) list.push({ id: other.id, ep: 'p2', ox: other.x2, oy: other.y2 });
      }
      return list;
    };

    return {
      wallId: w.id, mode, startWorld,
      origX1: w.x1, origY1: w.y1,
      origX2: w.x2, origY2: w.y2,
      // connAtP1: paredes ligadas ao endpoint p1 — seguem quando p1 mover
      connAtP1: (mode === 'body' || mode === 'p1') ? collectAt(w.x1, w.y1) : [],
      // connAtP2: paredes ligadas ao endpoint p2 — seguem quando p2 mover
      connAtP2: (mode === 'body' || mode === 'p2') ? collectAt(w.x2, w.y2) : [],
    };
  },

  // Aplica o drag na parede e nas adjacentes (chamado no touchMove)
  _applyWallDrag(d, raw) {
    const walls = this.project.canvas.walls;
    const w     = walls.find(w => w.id === d.wallId);
    if (!w || w.locked) return;

    const ddx = raw.x - d.startWorld.x;
    const ddy = raw.y - d.startWorld.y;

    let newX1 = d.origX1, newY1 = d.origY1;
    let newX2 = d.origX2, newY2 = d.origY2;

    if (d.mode === 'p1')   { newX1 += ddx; newY1 += ddy; }
    if (d.mode === 'p2')   { newX2 += ddx; newY2 += ddy; }
    if (d.mode === 'body') { newX1 += ddx; newY1 += ddy; newX2 += ddx; newY2 += ddy; }

    w.x1 = newX1; w.y1 = newY1;
    w.x2 = newX2; w.y2 = newY2;

    // Atualizar paredes adjacentes que estavam ligadas em p1
    for (const c of d.connAtP1) {
      const cw = walls.find(w => w.id === c.id);
      if (!cw) continue;
      if (c.ep === 'p1') { cw.x1 = newX1; cw.y1 = newY1; }
      else               { cw.x2 = newX1; cw.y2 = newY1; }
    }
    // Atualizar paredes adjacentes que estavam ligadas em p2
    for (const c of d.connAtP2) {
      const cw = walls.find(w => w.id === c.id);
      if (!cw) continue;
      if (c.ep === 'p1') { cw.x1 = newX2; cw.y1 = newY2; }
      else               { cw.x2 = newX2; cw.y2 = newY2; }
    }
  },
  ...
  _onTouchStart(e) {
    ...
    if (e.touches.length === 1) {
      ...
      // Select + parede selecionada: verificar se tocou num handle de endpoint ou corpo
      if (this.currentTool === 'select' && this.selected?.type === 'wall') {
        const w = this.project.canvas.walls.find(w => w.id === this.selected.id);
        if (w) {
          const HR    = 28 / this.zoom;
          const bodyR = 20 / this.zoom;

          if (dist(raw.x, raw.y, w.x1, w.y1) <= HR) {
            this._wallDrag = this._buildWallDrag(w, 'p1', raw);
            this._draw(); return;
          }
          if (dist(raw.x, raw.y, w.x2, w.y2) <= HR) {
            this._wallDrag = this._buildWallDrag(w, 'p2', raw);
            this._draw(); return;
          }
          if (this._pointNearSegment(raw, { x: w.x1, y: w.y1 }, { x: w.x2, y: w.y2 }, bodyR)) {
            if (!w.locked) {
              this._wallDrag = this._buildWallDrag(w, 'body', raw);
            }
            // Long press → travar/destravar
  ...
  _onTouchMove(e) {
    ...
    if (e.touches.length === 1 && this._touches.length === 1) {
      ...
      // Drag de parede — move parede + todas as adjacentes conectadas
      if (this._wallDrag) {
        if (this._longPressTimer) { clearTimeout(this._longPressTimer); this._longPressTimer = null; }
        this._applyWallDrag(this._wallDrag, raw);
        this._updateDragDimHud();
        this._touches = Array.from(e.touches);
        this._draw();
        return;
