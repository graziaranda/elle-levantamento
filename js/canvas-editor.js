  ...
  _drawWalls(ctx) {
    ...
    const chainPoly = (pts, t, closed) => {
      ...
      const miterPt = (i, sign) => {
        // i = índice do junction (entre seg[i-1] e seg[i])
        ...
        if (!isect) return p1;
        // Limitar miter excessivo (evita spikes em ângulos muito agudos)
        const dMiter = Math.sqrt((isect.x-P.x)**2 + (isect.y-P.y)**2);
        if (dMiter > h * 8) return p1; // fallback para bevel
        return isect;
      };
    ...
    for (const chain of chains) {
      ...
      // Para loop fechado, fechar a array de pontos para calcular miter no closing corner
      const polyPts = closed ? [...pts, pts[1]] : pts;
