...
const CanvasEditor = {
      ...
      <div class="canvas-editor">
        ...
        <header class="editor-header">
          ...
          <div style="display:flex;gap:6px;margin-left:10px;">
            <button class="btn-ghost" id="btn-undo" style="padding:6px 10px;" title="Desfazer">
              ${this._ic('undo')}
            </button>
  ...
  _drawDimensions(ctx) {
    for (const d of this.project.canvas.dimensions) {
      ...
      const label = val >= 1000 ? `${(val / 1000).toFixed(3).replace('.',',')}m` : `${val}mm`;
      const mx = (p1.x + p2.x) / 2;
      const my = (p1.y + p2.y) / 2;
      const fs = 15 / this.zoom;
      const u  = 1 / this.zoom;
      ...
      ctx.textAlign = 'center';
      // fundo (pílula) para legibilidade
      const tw = ctx.measureText(label).width;
      ctx.fillStyle = '#1A1814';
      ctx.fillRect(-tw / 2 - 5 * u, -fs - 3 * u, tw + 10 * u, fs + 5 * u);
      ctx.fillStyle = color;
  ...
  _drawPreview(ctx) {
    ...
    if (this.currentTool === 'wall') {
      ...
      ctx.lineWidth   = 150;
      ctx.lineCap     = 'square';
      ctx.setLineDash([500 / this.zoom, 200 / this.zoom]);
      ctx.beginPath();
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
      ctx.stroke();
    ...
    if (this.currentTool !== 'environment') {
      const r = 13 / this.zoom;        // ≈13px de raio independente do zoom
      // halo externo
  ...
  _onMouseDown(e) {
    ...
    // Parede: clica para fixar o início; depois arrasta para mirar
    if (this.currentTool === 'wall') {
      if (!this.drawStart) this._wallSetAnchor(world);
      else { this._aiming = true; this._wallAim(raw); }
      return;
    }

    // Porta/janela: arrasta até a parede (realça em tempo real)
    ...
    switch (this.currentTool) {
      case 'dimension':   this._clickDim(world);    break;
  ...
  _onMouseMove(e) {
    ...
    if (this.currentTool === 'wall' && this._aiming) { this._wallAim(raw); return; }
  ...
  _wallAim(rawWorld) {
    ...
    // Snap de fechamento: perto do 1º ponto do contorno
    if (first && this._chainPts.length >= 2 &&
        dist(rawWorld.x, rawWorld.y, first.x, first.y) * this.zoom < 30) {
  ...
  _numpad({ title, hint, value, unit = 'cm', onOk, onCancel }) {
    ...
    Modal.open(`
      <div class="numpad">
        ...
        <div class="numpad-grid">
          ...
          <button class="np-key np-clear" data-k="clear">C</button>
    ...
    document.querySelectorAll('.np-key').forEach(b => b.addEventListener('click', () => {
      const k = b.dataset.k;
      if (k === 'clear')      { buf = ''; fresh = false; }
      else if (k === 'back')  { if (fresh) { buf = ''; fresh = false; } else buf = buf.slice(0, -1); }
  ...
  _clickDim(world) {
    if (!this.drawStart) {
      ...
      this._updateCancelBtn();
      Toast.show('Ponto inicial da cota marcado — toque no ponto final', 'info', 3000);
    } else {
      const len = dist(this.drawStart.x, this.drawStart.y, world.x, world.y);
      if (len > 10) {
        this._pushHistory();
  ...
  // Realça a parede alvo + marca o canto de referência da medida
  _drawOpeningPlacement(ctx) {
    ...
    ctx.restore();
    // Canto de referência (de onde a distância é medida)
    if (pl.refX != null) {
  ...
  _showInstallModal(world) {
    const types = [
      { id: 'tomada110',   label: 'Tomada 110V',    css: 'electric' },
      { id: 'tomada220',   label: 'Tomada 220V',    css: 'electric' },
      { id: 'interruptor', label: 'Interruptor',     css: 'electric' },
      { id: 'luzTeto',     label: 'Luz teto',        css: 'electric' },
      { id: 'luzParede',   label: 'Luz parede',      css: 'electric' },
      { id: 'dados',       label: 'Ponto de dados',  css: 'electric' },
      { id: 'splitInterno',label: 'Split interno',   css: 'electric' },
      { id: 'splitExterno',label: 'Split externo',   css: 'electric' },
      { id: 'aguaFria',    label: 'Água fria',       css: 'hydro' },
      { id: 'aguaQuente',  label: 'Água quente',     css: 'hydro' },
      { id: 'ralo',        label: 'Ralo',            css: 'hydro' },
      { id: 'esgoto',      label: 'Saída esgoto',    css: 'hydro' },
      { id: 'gas',         label: 'Ponto de gás',    css: 'hydro' },
    ];

    const elec  = types.filter(t => t.css === 'electric');
    const hydro = types.filter(t => t.css === 'hydro');

    const gridHtml = (arr) => `
      <div class="install-grid">
        ${arr.map(t => `<button class="install-btn ${t.css}" data-type="${t.id}">${t.label}</button>`).join('')}
      </div>
    `;
    ...
    Modal.open(`
      ...
      <div class="modal-section">Elétrica</div>
      ${gridHtml(elec)}
      <div class="modal-section">Hidráulica / Gás</div>
      ${gridHtml(hydro)}
      <div class="modal-footer">
        <button class="btn-ghost" id="mc-cancel">Cancelar</button>
      </div>
    ...
    document.querySelectorAll('.install-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        Modal.close();
        const seq = this.project.canvas.installations.filter(i => this._instCode(i.type) === this._instCode(type)).length + 1;
        this._pushHistory();
        this.project.canvas.installations.push({
          id: generateId(), type, x: world.x, y: world.y,
          height: null, observation: '', sequenceNumber: seq,
        });
        this._scheduleSave();
        this._draw();
      });
  ...
  _showProps(elem, type) {
    ...
    if (type === 'wall') {
      ...
      document.getElementById('prop-thick').addEventListener('change', e => {
        const w = this.project.canvas.walls.find(w => w.id === elem.id);
    ...
    if (type === 'install') {
      ...
      document.getElementById('prop-height').addEventListener('change', e => {
        ...
        if (inst) { inst.height = parseInt(e.target.value) || null; this._scheduleSave(); this._draw(); }
      });
  ...
  _undo() {
    if (this.historyIdx < 0 || this.history.length === 0) {
      Toast.show('Nada para desfazer', 'info'); return;
    }
    this.historyIdx = Math.max(0, this.historyIdx - 1);
    this.project.canvas = JSON.parse(this.history[this.historyIdx]);
    // Desfazer encerra qualquer desenho de parede em andamento
    this.drawStart = null; this._chainPts = null;
    this._aiming = false; this._aimAngle = null; this._aimClosing = false;
    this._updateCancelBtn();
    this._scheduleSave();
    this._draw();
  },
  ...
  _instLabel(type) {
    const MAP = {
      tomada110: 'Tomada 110V', tomada220: 'Tomada 220V', interruptor: 'Interruptor',
