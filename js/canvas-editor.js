...
const CanvasEditor = {
  ...
  _installAim(rawWorld) {
    ...
    if (bestWall && bestDist <= 700 / this.zoom) {
      const refX = bestWall.x1 + (bestWall.x2 - bestWall.x1) * bestT;
      const refY = bestWall.y1 + (bestWall.y2 - bestWall.y1) * bestT;
      this._placingInstall = { wallId: bestWall.id, wallT: bestT, refX, refY };
    } else {
      this._placingInstall = null;
    }
    this._draw();
  ...
  _drawInstallPlacement(ctx) {
    ...
    ctx.beginPath(); ctx.moveTo(w.x1, w.y1); ctx.lineTo(w.x2, w.y2); ctx.stroke();
    ctx.restore();
    // Ponto azul na posição
    if (pl.refX != null) {
      ctx.fillStyle = '#5B9BD5';
      ctx.beginPath(); ctx.arc(pl.refX, pl.refY, 12 / this.zoom, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = '#1A1814'; ctx.lineWidth = 2 / this.zoom; ctx.stroke();
    }
  },
  ...
  _openInstallForm(wall, wallT) {
    ...
    const syncPreview = (type, distCm) => {
      ...
      // Atualiza o ponto azul
      this._placingInstall = { wallId: wall.id, wallT: tClamped, refX: px, refY: py };
      this._draw();
    };
    ...
    const showStep2 = (type) => {
      ...
      const distCm   = Math.round(wallT * wallLen / 10); // distância inicial em cm
