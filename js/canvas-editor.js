...
const CanvasEditor = {
  ...
  // Captura pontos estratégicos do levantamento com raio generoso (20px).
  // Pontos capturados: cantos de parede, meios de parede, bordas de vão,
  // centro de vão, posições de instalação.
  // Retorna { x, y, label } ou null se nenhum ponto próximo.
  _snapToArchPoint(rawWorld) {
    const R  = 20 / this.zoom;   // raio de captura em mm (20px de tela)
    const c  = this.project.canvas;
    const candidates = [];

    // Cantos e meios de cada parede
    for (const w of c.walls) {
      candidates.push({ x: w.x1, y: w.y1, label: 'Canto' });
      candidates.push({ x: w.x2, y: w.y2, label: 'Canto' });
      candidates.push({
        x: (w.x1 + w.x2) / 2, y: (w.y1 + w.y2) / 2, label: 'Meio'
      });
    }
    ...
    return best; // null = nenhum snap arquitetônico
  },
  ...
  _onMouseMove(e) {
    ...
    // Snap magnético de cota: atualiza indicador em tempo real
    if (this.currentTool === 'dimension') {
  ...
  _installAim(rawWorld) {
    ...
    if (bestWall && bestDist <= 700 / this.zoom) {
      // Referência = canto mais próximo da parede (start ou end), não onde o dedo soltou.
      // O ponto azul fica sempre fixo num canto — só muda ao ⇄ para o outro canto.
      ...
      this._placingInstall = {
        ...
        wallT:      bestT,
        nearStart,
        // ponto azul = canto fixo (start ou end)
        refX: nearStart ? bestWall.x1 : bestWall.x2,
        refY: nearStart ? bestWall.y1 : bestWall.y2,
        // distância inicial = onde o dedo soltou (pré-preenchimento conveniente)
  ...
  _onTouchMove(e) {
    ...
    if (e.touches.length === 1 && this._touches.length === 1) {
      ...
      // Snap magnético de cota no touch move
      if (this.currentTool === 'dimension') {
        this._archSnapPt = this._snapToArchPoint(raw);
        if (this._archSnapPt) this.mouseWorld = this._archSnapPt;
      } else {
